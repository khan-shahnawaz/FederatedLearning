...
class Server(BaseFedarated):
    ...
    def train(self):
        ...
        for i in range(len(self.clients)):
            batch = gen_batch(self.clients[i].train_data, self.batch_size, self.local_iters)
            self.clients[i].set_params(self.latest_model)
            for _ in range(self.local_iters):
                data_batch = next(batch)
                _, grads, _ = self.clients[i].solve_sgd(data_batch)
        num_test, num_correct_test, test_loss_vector = self.test()  # have set the latest model for all clients
        avg_test_loss = np.dot(test_loss_vector, num_test) / np.sum(num_test)
        num_train, num_correct_train, train_loss_vector = self.train_error()
        avg_train_loss = np.dot(train_loss_vector, num_train) / np.sum(num_train)

        tqdm.write('Finetuned training accu: {}, loss: {}'.format(np.sum(num_correct_train) * 1.0 / np.sum(
            num_train), avg_train_loss))
        tqdm.write('Finetuned test loss: {}'.format(avg_test_loss))
        tqdm.write('Finetuned test accu: {}'.format(np.sum(num_correct_test) * 1.0 / np.sum(num_test)))
        non_corrupt_id = np.setdiff1d(range(len(self.clients)), corrupt_id)
        tqdm.write('Finetuned malicious test accu: {}'.format(np.sum(
            num_correct_test[corrupt_id]) * 1.0 / np.sum(num_test[corrupt_id])))
        tqdm.write('Finetuned benign test accu: {}'.format(np.sum(num_correct_test[non_corrupt_id]) * 1.0 / np.sum(num_test[non_corrupt_id])))
        tqdm.write('Finetuned variance: {}'.format(np.var(
            num_correct_test[non_corrupt_id] * 1.0 / num_test[non_corrupt_id])))
