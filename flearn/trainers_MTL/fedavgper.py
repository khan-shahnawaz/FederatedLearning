...
class Server(BaseFedarated):
    def __init__(self, params, learner, dataset):
        print('Using FedAvgPer multi-task learning to Train')
    ...
    def train(self):
        ...
        for i in range(self.num_rounds + 1):
            ...
            for idx in len(self.clients):
                ...
                for _ in range(self.local_iters):
                    ...
                    ''' Perform local iterations to send it to the server'''
                    # global
        ...
        ''' Running local iterations on each client after the global model is completed'''
        self.lam = 0    #No constraints on the local updates
        for idx in range(len(self.clients)):
            self.client_model.set_params(copy.deepcopy(self.global_model))  #Copy the global model to the client model
            c = self.clients[idx]
            for _ in range(self.local_iters):
                data_batch = next(batches[c])
                _, grads, _ = c.solve_sgd(data_batch)  
        ...
        avg_train_loss = np.dot(loss_vector, num_train) / np.sum(num_train)
        num_test, num_correct_test, _ = self.test(tmp_models)
        tqdm.write('Final training accu: {}, loss: {}'.format(i, np.sum(num_correct_train) * 1.0 / np.sum(num_train), avg_train_loss))
        tqdm.write('Final test accu: {}'.format(i, np.sum(num_correct_test) * 1.0 / np.sum(num_test)))
        non_corrupt_id = np.setdiff1d(range(len(self.clients)), corrupt_id)
        tqdm.write('Final malicious test accu: {}'.format(i, np.sum(num_correct_test[corrupt_id]) * 1.0 / np.sum(num_test[corrupt_id])))
        tqdm.write('Final benign test accu: {}'.format(i, np.sum(num_correct_test[non_corrupt_id]) * 1.0 / np.sum(num_test[non_corrupt_id])))
        print("variance of the performance: ", np.var(num_correct_test[non_corrupt_id] / num_test[non_corrupt_id]))