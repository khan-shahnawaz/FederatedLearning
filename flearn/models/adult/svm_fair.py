...
class Model(object):    
    ...
    def create_model(self, q, optimizer):
        ...
        num_males = tf.reduce_sum(tf.cast(tf.equal(features[:, 3], 1), tf.float32))
        ...
        loss = 0.01 * tf.reduce_sum(tf.square(W)) + tf.reduce_mean(
            tf.maximum(tf.zeros_like(labels), (1 - labels * y_pred))*tf.maximum(2*(2*features[:,3]-1), 3*(1-(2*features[:,3]-1)))
        )
