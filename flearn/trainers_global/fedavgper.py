...
class Server(BaseFedarated):
    def __init__(self, params, learner, dataset):
        print('Using fedavgper to Train')
    ...
    def train(self):
        ...
        #After global training is complete, train on local data to finetune the model
        NUM_LOCAL_UPDATES = 10     #Number of local updates to perform
        ''' Generate new batches for each client '''
        for idx, c in enumerate(self.clients):
            if idx in corrupt_id:
                c.train_data['y'] = np.asarray(c.train_data['y'])
                if self.dataset == 'celeba':
            ...
            if self.dataset == 'celeba':  # need to deal with celeba data loading a bit differently
                batches[c] = gen_batch_celeba(c.train_data, self.batch_size, NUM_LOCAL_UPDATES)
            else:
                batches[c] = gen_batch(c.train_data, self.batch_size, NUM_LOCAL_UPDATES)
        for i,c in enumerate(self.clients):
            c.set_params(self.latest_model)
            for _ in range(NUM_LOCAL_UPDATES):
                data_batch = next(batches[c])
                _, grads, _ = c.solve_sgd(data_batch)
        
        #Get the results after finetuning
        num_test, num_correct_test, test_loss_vector = self.test()  # have set the latest model for all clients
        avg_test_loss = np.dot(test_loss_vector, num_test) / np.sum(num_test)
        num_train, num_correct_train, train_loss_vector = self.train_error()
        avg_train_loss = np.dot(train_loss_vector, num_train) / np.sum(num_train)
        
        tqdm.write('After finetuning training accu: {}, loss: {}'.format(np.sum(num_correct_train) * 1.0 / np.sum(num_train), avg_train_loss))
        tqdm.write('After finetuning test loss: {}'.format(avg_test_loss))
        tqdm.write('After finetuning test accu: {}'.format(np.sum(num_correct_test) * 1.0 / np.sum(num_test)))
        non_corrupt_id = np.setdiff1d(range(len(self.clients)), corrupt_id)
        tqdm.write('After finetuning malicious test accu: {}'.format(np.sum(num_correct_test[corrupt_id]) * 1.0 / np.sum(num_test[corrupt_id])))
        tqdm.write('After finetuning benign test accu: {}'.format(np.sum(num_correct_test[non_corrupt_id]) * 1.0 / np.sum(num_test[non_corrupt_id])))
        tqdm.write('After finetuning variance: {}'.format(np.var(num_correct_test[non_corrupt_id] * 1.0 / num_test[non_corrupt_id])))